...
def extract_education(nlp_text):
    ...
    # Extract education degree
     # Convert tokens to string and process each sentence separately
    nlp_text_str = [sent.text for sent in nlp_text.sents]  # Extract sentences from the Doc object

    for index, sentence in enumerate(nlp_text_str):
        words = sentence.split()  # Split the sentence into words
        for word in words:
            cleaned_word = re.sub(r'[?|$|.|!|,]', r'', word)  # Remove special characters
            if cleaned_word.upper() in cs.EDUCATION and cleaned_word not in cs.STOPWORDS:
                edu[cleaned_word] = sentence

    # Extract year
    education = []
    for key in edu.keys():
        year_match = re.search(re.compile(cs.YEAR), edu[key])
        if year_match:
            education.append((key, ''.join(year_match.group(0))))
        else:
